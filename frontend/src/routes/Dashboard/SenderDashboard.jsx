...
import { FaFileUpload, FaSpinner, FaFileAlt, FaInbox } from 'react-icons/fa';
import { z } from 'zod';
...
export default function SenderDashboard() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await api.post('/sender/create-record', payload, {
        ...
        <div className="space-y-8">
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">New Record Entry</h2>
                </div>
                ...
                <div className="mt-6 flex justify-end">
                    <motion.button
                        ...
                        className="px-6 py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-medium rounded-xl hover:opacity-90 transition-opacity flex items-center shadow-sm"
                    >
            ...
            >
                <div className="p-6 border-b border-gray-200/80 dark:border-gray-800/80 flex justify-between items-center">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white tracking-tight">My Records</h2>
                </div>

                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50/80 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 uppercase tracking-wider font-semibold text-xs">
                            <tr>
                        ...
                        <tbody className="divide-y divide-gray-200/80 dark:divide-gray-800/80">
                            {loadingRecords ? (
                                ...
                                <tr><td colSpan="5" className="px-6 py-12 text-center"><div className="flex flex-col items-center gap-2 text-gray-500 dark:text-gray-400"><FaInbox className="w-10 h-10 opacity-50" /><span>No records yet. Create one above.</span></div></td></tr>
                            ) : (
                                records.map((record, i) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 font-medium text-gray-900 dark:text-white flex items-center">
                                            <FaFileAlt className="text-gray-400 mr-3 shrink-0" />
                                        ...
                                        <td className="px-6 py-4 text-gray-500 dark:text-gray-400">{new Date(record.createdAt).toLocaleDateString()}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center text-green-600 dark:text-green-400 text-xs font-bold uppercase tracking-wide">
                                                <span className="w-2 h-2 bg-green-500 rounded-full mr-2"></span>
                                                Active
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <Link to={`/dashboard/records/${record._id}`} className="text-blue-600 dark:text-blue-400 hover:underline font-medium">View</Link>
                                        </td>
                                    </motion.tr>
                                ))
                            )}
